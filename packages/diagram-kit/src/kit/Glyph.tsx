 ...
 *
 * Adding a glyph: draw it as a `GlyphRender` in the matching category
 * module under `glyphs/` following the style spec above. Names are
 * nouns/actors (user, server, wallet) - verbs and flows stay arrows
 * with labels.
 */
...
export type GlyphName = CoreGlyphName | SoftwareGlyphName;

const deg2rad = (d: number) => (d * Math.PI) / 180;

const CORE_GLYPHS: Record<CoreGlyphName, GlyphRender> = {
  user: ({ accent, ink }) => (
    <g>
      <path
  ...
  "vector-db": ({ accent, light, ink }) => (
    <g>
      <path
        d="M 18 20 L 18 58 A 22 10 0 0 0 62 58 L 62 20"
        fill={accent}
        stroke={ink}
        strokeWidth={4}
      />
  ...
  gauge: ({ accent, light, ink }) => (
    <g>
      <path
        d="M 12 56 A 28 28 0 0 1 68 56 Z"
        fill={light}
        stroke={ink}
        strokeWidth={4}
        strokeLinejoin="round"
